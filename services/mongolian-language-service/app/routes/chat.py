...
CYRILLIC_RE = re.compile(r"[А-Яа-яӨөҮүЁё]")
...
def missing_fields(context: dict) -> list[str]:
    return [field for field in REQUIRED_APPLICATION_FIELDS if not context.get(field)]
...
async def chat(payload: ChatRequest, request: Request) -> ChatResponse:
    ...
    language = detect_dominant_language(latest_user)
    ...
    system = (
        ...
        "mixed, answer in the dominant language. Use polite 'Та' for Mongolian borrowers. "
